...
lastspace<-function(a){
  b=""
  if(nchar(a)>1){b=strsplit(a,"")[[1]]}
  if(b[length(b)]==" ")return(TRUE)
  else return(FALSE)
}
...
predictword1<-function(sentence,candidates=NULL){
  ...
  if(!is.null(candidates)){##there is candidates
    if(nrow(r)!=0)
    {   ## hints from N gram
      if(nrow(r[r$prediction%in%candidates])!=0)
      { ##"n gram prediction found in candidates
        prediction=r[r$prediction%in%candidates]
        if(nrow(prediction)>1)prediction=prediction[order(p,decreasing=T)]
        return(unique(prediction$prediction))
      }
    ...
    if(nrow(r)!=0){
      ...
      prediction=r
      if(nrow(prediction)>1)prediction=prediction[order(p,decreasing=T)]
      if(length(unique(prediction$prediction))>5)return((unique(prediction$prediction))[1:5])
      return (unique(prediction$prediction))
    }
    else 
    {
...
predictword2<-function(sentence,candidates=NULL){
  ...
  if(!is.null(candidates)){##there is candidates
    if(nrow(r)!=0)
    {   ## hints from N gram
      if(nrow(r[r$prediction%in%candidates])!=0)return("N gram prediction with candidates")
      else { 
        u=uni_gram$uni_gram
        predict=uni_gram[u%in%candidates]
        predict=predict[order(Frequency,decreasing=T)]
        ...
        else return("Cannot predict next word from candidates. Please predict without candidates!")
      }
    }
    else{## one gram prediction
      u=uni_gram$uni_gram
    ...
    {
      return("Cannot predict next words. Stopwords are suggested")
    }
  }
}
  ...
  function(input, output,session) {
    observe({
      ...
      if(input$button1 == '1') {
        ...
        session$sendCustomMessage(type="jsCode",
                                  list(code= "$('#submit2').prop('disabled',true)"))
        ...
        session$sendCustomMessage(type="jsCode",
                                  list(code= "$('#submit2').prop('disabled',false)"))
    ...
    output$outputtext2<-eventReactive(input$submit2,{
        a=input$sentence
        candidates=NULL
        if(input$button1 == '1'){
          candidates=NULL
        ...
        p=paste("(",1:length(p),")",p)
   
  })
     
   
    
    output$outputtext<-eventReactive(input$submit1||lastspace(input$sentence),{
      if(lastspace(input$sentence)||input$submit1>0){
      a=input$sentence
      candidates=NULL
      p=predictword1(a,NULL)
      p=paste("(",1:length(p),")",p)
      p
      }
    ...
    output$hints2<-eventReactive(input$submit2,{
       a=input$sentence
       candidates=NULL
       if(input$button1 == '1'){
         candidates=NULL
       ...
       p=predictword2(a,candidates)
      
     })
     
    output$hints<-eventReactive(input$submit1||lastspace(input$sentence),{
      if(lastspace(input$sentence)||input$submit1>0){
        a=input$sentence
        candidates=NULL
        p=predictword2(a,NULL)
